...
tf.app.flags.DEFINE_string('dataset', '/tungstenfs/scratch/gzenke/agarmanv/data/TIMIT_processed', 'Path to dataset to use')
...
tf.app.flags.DEFINE_integer('print_every', 100, 'print every n iterations and store accuracy; validation periodicity')
...
with tf.variable_scope('RNNs'):
    def bi_directional_lstm(inputs, layer_number):
        ...
        with tf.variable_scope('BiDirectionalLayer' + str(layer_number)):
            ...
            # size: batch_size x seq_len x n_rec
...
def compute_result(type="validation"):
    ...
    print("Running dataset type: ", type)
...
while dataset.current_epoch <= FLAGS.n_epochs:
    ...
    print("Epoch: ", dataset.current_epoch)
    ...
    if np.mod(k_iteration, FLAGS.print_every) == 0:
        ...
        print("Running validation and test sets...")
